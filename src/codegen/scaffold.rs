...
pub fn scaffold_repo(manifest: &Manifest, output_dir: &Path) -> ScaffoldResult {
    ...
    // Unified transaction-gated adapter (single loopback port, SSE, standards#89 sub-issue 1)
    files.push(generate_unified_adapter(&model, &iser_name));
...
// ---------------------------------------------------------------------------
// File generators — CI/CD (regeneration trigger + unified adapter)
// ---------------------------------------------------------------------------
...
/// Generate `adapter/<iser_name>_adapter.zig` — the unified transaction-gated
/// BoJ adapter (single loopback listener, SSE + REST + GraphQL + gRPC-compat,
/// one gated dispatch → one Zig ABI).
///
/// Replaces the ssg-era 3-parallel-port adapter estate-wide as of standards#89
/// sub-issue 1 (pilot: k9iser-mcp, boj-server#73).  Internal-only per ADR-0004:
/// the public surface is the http-capability-gateway (tier-2).
fn generate_unified_adapter(model: &LanguageModel, iser_name: &str) -> GeneratedFile {
    let content = format!(
        r#"// SPDX-License-Identifier: PMPL-1.0-or-later
// Copyright (c) 2026 Jonathan D.A. Jewell (hyperpolymath) <j.d.a.jewell@open.ac.uk>
//
// {iser_name}/adapter/{iser_name}_adapter.zig
//
// INTERNAL-ONLY unified transaction-gated adapter.  Per ADR-0004 the only
// governed public surface is the http-capability-gateway (tier-2) in front;
// cartridge adapters bind loopback and sit behind it.
//
//   One listener, one port, protocol-routed:
...
// Idris2 `exposureSatisfied` contract) BEFORE dispatch.  No request reaches
// the ABI ungated — estate interface-safety policy.
//
// Generated by iseriser for {lang_name} (standards#89 sub-issue 1).
// Pattern source: k9iser-mcp, boj-server#73.

const std = @import("std");
const ffi = @import("{iser_name}_ffi");

// Loopback-only by construction: fronted by http-capability-gateway (ADR-0004).
const BIND_IP = [4]u8{{ 127, 0, 0, 1 }};
const PORT: u16 = 9390; // Override in cartridge manifest if port conflicts.

// ── Transaction gate (mirrors Idris2 {module_name}.exposureSatisfied) ──────
//
// Encoding: 0=Public 1=Authenticated 2=Internal.
const Exposure = enum(u8) {{ public = 0, authenticated = 1, internal = 2 }};

// Default: require Public (for unauthenticated cartridges).
// Set to .authenticated for credential-bearing cartridges.
...
/// Parse the `X-Trust-Level` header the gateway/sidecar injects.
...
fn dispatchTool(tool: []const u8, body: []const u8, resp: []u8) struct {{ status: u16, body: []u8 }} {{
    _ = body;
    // Route to the {iser_name}_ffi ABI symbol.  Extend as tools are added.
    _ = tool;
    const msg = std.fmt.bufPrint(resp, "{{{{\"tool\":\"{iser_name}\",\"status\":\"dispatched\"}}}}", .{{}}) catch resp[0..0];
    return .{{ .status = 200, .body = msg }};
}}

fn dispatchRest(path: []const u8, body: []const u8, is_local: bool, req: []const u8, resp: []u8) struct {{ status: u16, body: []u8 }} {{
    const presented = presentedExposure(req);
    if (!exposureSatisfied(REQUIRED_EXPOSURE, presented, is_local)) {{
        const msg = std.fmt.bufPrint(resp, "{{{{\"error\":\"forbidden\"}}}}", .{{}}) catch resp[0..0];
        return .{{ .status = 403, .body = msg }};
    }}
    const prefix = "/invoke";
    if (std.mem.startsWith(u8, path, prefix)) {{
        // Extract tool name from JSON body field "tool".
        const tool = jsonField(body, "tool") orelse "unknown";
        return dispatchTool(tool, body, resp);
    }}
    const msg = std.fmt.bufPrint(resp, "{{{{\"error\":\"not found\"}}}}", .{{}}) catch resp[0..0];
    return .{{ .status = 404, .body = msg }};
}}

fn dispatchSse(body: []const u8, is_local: bool, req: []const u8, stream: std.net.Stream) void {{
    const presented = presentedExposure(req);
    if (!exposureSatisfied(REQUIRED_EXPOSURE, presented, is_local)) {{
        _ = stream.write("HTTP/1.1 403 Forbidden\r\n\r\n") catch {{}};
        return;
    }}
    const tool = jsonField(body, "tool") orelse "unknown";
    _ = stream.write("HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nCache-Control: no-cache\r\n\r\n") catch {{}};
    var buf: [256]u8 = undefined;
    const event = std.fmt.bufPrint(&buf, "data: {{{{\"tool\":\"{iser_name}\",\"dispatched\":\"{{}}\"}}}}\n\n", .{{tool}}) catch &buf;
...
fn handleConnection(stream: std.net.Stream) void {{
    ...
    const is_local = true; // All connections via loopback are locally trusted.

    const path_start = std.mem.indexOf(u8, req, " ") orelse return;
    const path_end = std.mem.indexOf(u8, req[path_start + 1 ..], " ") orelse return;
    const path = req[path_start + 1 .. path_start + 1 + path_end];

    var resp_buf: [2048]u8 = undefined;

    if (std.mem.startsWith(u8, path, "/sse")) {{
        const body = extractBody(req);
        dispatchSse(body, is_local, req, stream);
    }} else {{
        const body = extractBody(req);
        const result = dispatchRest(path, body, is_local, req, &resp_buf);
        const http = std.fmt.bufPrint(&resp_buf, "HTTP/1.1 {{}} OK\r\nContent-Type: application/json\r\n\r\n{{s}}", .{{ result.status, result.body }}) catch &resp_buf;
...
pub fn main() !void {{
    ...
    defer server.deinit();
    std.log.info("{iser_name} unified adapter listening on 127.0.0.1:{{}} (loopback-only, ADR-0004)", .{{PORT}});
    while (true) {{
...
fn headerValue(req: []const u8, name: []const u8) ?[]const u8 {{
    ...
    while (lines.next()) |line| {{
        if (eqIgnoreCase(line[0..@min(name.len, line.len)], name)) {{
            const col = std.mem.indexOf(u8, line, ":") orelse continue;
            return std.mem.trim(u8, line[col + 1 ..], &std.ascii.whitespace);
        }}
    }}
    return null;
}}
...
fn jsonField(body: []const u8, key: []const u8) ?[]const u8 {{
    ...
    const needle = std.fmt.bufPrint(&search_buf, "\"{{}}\": \"", .{{key}}) catch return null;
...
fn extractBody(req: []const u8) []const u8 {{
    const sep = "\r\n\r\n";
    const pos = std.mem.indexOf(u8, req, sep) orelse return req;
    return req[pos + sep.len ..];
}}

// ── Tests ────────────────────────────────────────────────────────────────────

test "exposureSatisfied: loopback always passes" {{
    try std.testing.expect(exposureSatisfied(.internal, .public, true));
    try std.testing.expect(exposureSatisfied(.authenticated, .public, true));
}}

test "exposureSatisfied: public gate passes all" {{
    try std.testing.expect(exposureSatisfied(.public, .public, false));
    try std.testing.expect(exposureSatisfied(.public, .authenticated, false));
    try std.testing.expect(exposureSatisfied(.public, .internal, false));
}}

test "exposureSatisfied: authenticated gate rejects public" {{
...
test "exposureSatisfied: internal gate rejects public and authenticated" {{
    try std.testing.expect(!exposureSatisfied(.internal, .public, false));
...
"#,
        iser_name = iser_name,
        lang_name = model.name,
        module_name = idris2_module_name(model),
    );
    GeneratedFile {
        path: PathBuf::from(format!("adapter/{iser_name}_adapter.zig")),
        content,
    }
}
...
"#;
    ...
    fn test_scaffold_generates_all_files() {
        ...
        // 22+ files: 20 base + regen workflow + unified adapter (standards#89 sub-issue 1)
        assert!(
            repo.file_count() >= 22,
    ...
    fn test_files_written_to_disk() {
        ...
        // standards#89 sub-issue 1: regen trigger + unified adapter
        assert!(repo_root.join(".github/workflows/chapeliser-regen.yml").exists());
        assert!(repo_root.join("adapter/chapeliser_adapter.zig").exists());
