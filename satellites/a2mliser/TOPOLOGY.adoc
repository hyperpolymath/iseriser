...
* `+src/interface/abi/*.idr+` *defines* the formal specification
(Idris2)
* `+src/interface/ffi/src/main.zig+` *implements* the specification
(Zig)
...
[arabic]
. Machine-readable files live in `+.machine_readable/+` ONLY — never in
root
. Idris2 ABI is the specification; Zig FFI is the implementation
. Generated C headers go in `+src/interface/generated/abi/+`
. All workflows are SHA-pinned, all code is MPL-2.0
